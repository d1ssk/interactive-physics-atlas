...
最高ウェイト・エクスプローラーには、名称付きのプリセットを選んで収録しています。
テンソル積エクスプローラーでも、初期アプリケーションを軽量かつ応答性の高いものに保ち、
Pythonサーバーを不要にするため、小さな表現を選んでいます。これらは現時点のインターフェースと
計算上の安全域による制限であり、数学的な制限ではありません。
