"""Build the self-hosted Pyodide runtime and scalar-vacuum kernel wheel."""
...
def runtime_manifest() -> dict[str, object]:
    """Return lightweight runtime configuration embedded in the HTML shell."""

    return {
        "protocol": COMPUTE_PROTOCOL_SCHEMA,
        "operation": SAMPLE_OPERATION,
        "resultSchema": SAMPLE_RESULT_SCHEMA,
...
def build_kernel_wheel(source_dir: Path, output_path: Path) -> None:
    """Build a deterministic pure-Python wheel from authoritative sources."""

    dist_info = f"{KERNEL_DISTRIBUTION}-{KERNEL_VERSION}.dist-info"
    entries: dict[str, bytes] = {
        f"{KERNEL_DISTRIBUTION}/__init__.py": (
            '"""Interactive Physics Olio scalar-vacuum kernel."""\n\n'
            f'__version__ = "{KERNEL_VERSION}"\n'
        ).encode(),
        f"{dist_info}/METADATA": (
            "Metadata-Version: 2.1\n"
            "Name: physics-atlas-scalar-vacuum-kernel\n"
            f"Version: {KERNEL_VERSION}\n"
            "Summary: Free-scalar vacuum sampler for Interactive Physics Olio\n"
            "License: MIT\n"
        ...
        f"{dist_info}/WHEEL": (
            b"Wheel-Version: 1.0\n"
            b"Generator: Interactive Physics Olio\n"
...
def build_runtime_assets(output_dir: Path, source_dir: Path) -> Path:
    """Stage provider, Worker, minimal Pyodide/NumPy assets, and kernel wheel."""
    ...
    replacements = {
        ...
        "__SAMPLE_OPERATION__": SAMPLE_OPERATION,
        "__SAMPLE_RESULT_SCHEMA__": SAMPLE_RESULT_SCHEMA,
    ...
    if any(placeholder in worker for placeholder in replacements):
        raise ValueError("unresolved scalar-vacuum Worker placeholder")
