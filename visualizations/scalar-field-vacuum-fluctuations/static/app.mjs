...
const TEXT = {
  en: {
    title:"Quantum Fluctuations of the Scalar-Field Vacuum",
    ...
    controlsText:"Changing the mass, cutoff, or seed generates a new pair of realizations.",
    ...
    spaceText:"This is an independent draw from the three-spatial-dimensional vacuum measure.",
    ...
    correlationTitle:"Sampled dimension-dependent correlations",
    correlationText:"Each point is a mean over 48 additional independent realizations; the error bar is one standard error of that mean.",
    ...
    sampleText:"Each picture is one field configuration drawn from the vacuum wavefunctional, equivalently from the free-vacuum Wigner distribution for canonical data.",
    timeHeading:"Free-field spacetime history",
    timeText:"A real probability distribution needs a real symmetric covariance, supplied here by the Hadamard function. Feynman ordering and Wightman operator ordering generally give complex or nonsymmetric functions, so they cannot directly be covariances of real random variables.",
  ...
  ja: {
    ...
    controlsText:"質量、カットオフ、seed を変えると、二つの配位を新たに生成します。",
    massLabel:"質量 \\(m\\) [\\(L^{-1}\\)]",
    cutoffLabel:"UV window \\(\\Lambda/\\Lambda_{\\rm Ny}\\)",
    ...
    spaceText:"空間3次元の真空 measure から独立に生成した配位です。",
    ...
    correlationTitle:"空間次元ごとの相関をサンプリング",
    correlationText:"各点は別に生成した48個の独立な配位についての平均で、error bar はその平均の標準誤差です。",
    correlationAria:"空間2次元と3次元の規格化した等時刻相関の比較",
    readingTitle:"この表示が表すもの・表さないもの",
    sampleHeading:"測定結果の分布",
    sampleText:"各図は、真空波動汎関数（正準変数については自由真空の Wigner 分布と等価）から場の配位を一つ取り出したものです。",
    timeHeading:"自由場の時空履歴",
    timeText:"実確率分布の共分散には、実数かつ対称な関数が必要で、ここでは Hadamard 関数がそれに当たります。Feynman 関数は時間順序、Wightman 関数は演算子順序を保つため一般には複素数値または非対称で、そのまま実確率変数の共分散にはできません。",
  ...
  const correlation = [
    {
      ...
      error_y:{
        type:"data",
        array:result.correlation.dimension2Error,
        visible:true,
        color:COLORS.negative,
        thickness:1.4,
        width:4,
      },
      marker:{color:COLORS.negative, size:7, symbol:"circle"},
      name:`d = 2 · N = ${result.correlation.sampleCount}`,
      hovertemplate:"r=%{x:.2f}<br>mean C₂(r)/C₂(0)=%{y:.3f}<br>SE=%{error_y.array:.3f}<extra></extra>",
    },
    {
      type:"scatter",
      mode:"markers",
      ...
      error_y:{
        type:"data",
        array:result.correlation.dimension3Error,
        visible:true,
        color:COLORS.dimension3,
        thickness:1.4,
        width:4,
      },
      marker:{
        color:COLORS.panel,
        line:{color:COLORS.dimension3, width:2},
        size:8,
        symbol:"diamond",
      },
      name:`d = 3 · N = ${result.correlation.sampleCount}`,
      hovertemplate:"r=%{x:.2f}<br>mean C₃(r)/C₃(0)=%{y:.3f}<br>SE=%{error_y.array:.3f}<extra></extra>",
    },
  ];
  await Promise.all([
