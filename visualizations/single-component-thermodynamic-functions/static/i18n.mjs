...
const STRINGS = {
  ...
  ja: {
    ...
    titleU: "内部エネルギー", titleS: "エントロピー", titleF: "Helmholtz 自由エネルギー", titleH: "エンタルピー", titleG: "Gibbs 自由エネルギー",
    computing: "局所と全体の熱力学曲面を計算しています…",
    directOrigin: "状態の直接指定", equilibriumOrigin: "物理操作 / 平衡状態",
    metricT: "温度 T", metricP: "圧力 P", metricV: "体積 v", metricS: "エントロピー s", metricU: "内部エネルギー u", metricF: "Helmholtz 自由エネルギー f", metricH: "エンタルピー h", metricG: "Gibbs 自由エネルギー g", metricMu: "化学ポテンシャル μ (= g)",
...
const STATIC_EN = new Map([
  ...
  ["熱力学では、巨視的な平衡状態を少数の状態変数で記述します。物質量を固定した一成分の単純圧縮系では、たとえばモルエントロピー \\(s\\) とモル体積 \\(v\\) を独立変数として選び、モル内部エネルギーを \\(u=u(s,v)\\) と表せます。温度 \\(T\\) や圧力 \\(P\\) を含む他の熱力学量は、この基本関係から導かれます。内部エネルギーやエンタルピーなどは平衡状態だけで決まる状態関数ですが、熱 \\(Q\\) と仕事 \\(W\\) は状態間の移り方に依存する経路量です。",
   "Thermodynamics describes macroscopic equilibrium states using a small number of state variables. For a one-component simple compressible system at fixed amount of substance, one may choose the molar entropy \\(s\\) and molar volume \\(v\\) as independent variables and write the molar internal energy as \\(u=u(s,v)\\). Other thermodynamic quantities, including the temperature \\(T\\) and pressure \\(P\\), follow from this fundamental relation. Internal energy, enthalpy, and the other thermodynamic potentials are state functions determined only by the equilibrium state, whereas heat \\(Q\\) and work \\(W\\) depend on the path taken between states."],
  ["内部エネルギー表示では、基本的な熱力学関係は \\(du=T\\,ds-P\\,dv\\) です。したがって、\\((\\partial u/\\partial s)_v=T\\)、\\((\\partial u/\\partial v)_s=-P\\) となり、\\(u(s,v)\\) の曲面の傾きから温度と圧力を読み取れます。同じ平衡状態の集合は、基本関係を \\(s=s(u,v)\\) と解き直したエントロピー表示でも記述でき、この場合は \\(ds=du/T+(P/T)\\,dv\\) となります。",
   "In the internal-energy representation, the fundamental thermodynamic relation is \\(du=T\\,ds-P\\,dv\\). Thus \\((\\partial u/\\partial s)_v=T\\) and \\((\\partial u/\\partial v)_s=-P\\), so the slopes of the surface \\(u(s,v)\\) directly give the temperature and pressure. The same set of equilibrium states can instead be described in the entropy representation by solving the fundamental relation for \\(s=s(u,v)\\), for which \\(ds=du/T+(P/T)\\,dv\\)."],
  ["状況に応じて、自然変数の異なる熱力学ポテンシャルを使うと便利です。モル Helmholtz 自由エネルギー \\(f=u-Ts\\)、モルエンタルピー \\(h=u+Pv\\)、モル Gibbs 自由エネルギー \\(g=u-Ts+Pv\\) は、\\(u(s,v)\\) の変数をルジャンドル変換によって取り替えたものです。それぞれ \\(df=-s\\,dT-P\\,dv\\)、\\(dh=T\\,ds+v\\,dP\\)、\\(dg=-s\\,dT+v\\,dP\\) を満たし、\\(f(T,v)\\)、\\(h(s,P)\\)、\\(g(T,P)\\) の曲面の傾きから、エントロピー・圧力・温度・体積といった共役な熱力学量を読み取れます。どのポテンシャルが使いやすいかは、温度、圧力、体積、エントロピーのうち何を外部から制御するかによって変わります。",
   "Different thermodynamic potentials are useful under different choices of externally controlled variables. The molar Helmholtz free energy \\(f=u-Ts\\), molar enthalpy \\(h=u+Pv\\), and molar Gibbs free energy \\(g=u-Ts+Pv\\) are obtained from \\(u(s,v)\\) by Legendre transformations that exchange natural variables. They satisfy \\(df=-s\\,dT-P\\,dv\\), \\(dh=T\\,ds+v\\,dP\\), and \\(dg=-s\\,dT+v\\,dP\\), so the slopes of the surfaces \\(f(T,v)\\), \\(h(s,P)\\), and \\(g(T,P)\\) encode conjugate quantities such as entropy, pressure, temperature, and volume. Which potential is most convenient depends on which of temperature, pressure, volume, or entropy are externally constrained."],
  ["この visualization では、ピストンによる体積変化、加熱・冷却、熱浴との接触、外圧の変更を通して、系が異なる平衡状態へ移る様子を熱力学関数の曲面上で追います。準静的極限では、過程そのものを平衡状態の連続的な軌跡としてみなすことができます。一方、有限の温度差や圧力差によって進む不可逆過程では、中間状態が必ずしも平衡状態とは限らないため、曲面上の点は主として操作の前後に成立する平衡状態を表します。収支には系が受け取った熱 \\(Q\\) と系が外界にした仕事 \\(W\\) が表示され、符号規約 \\(\\Delta U=Q-W\\) によって熱力学第一法則を確認できます。",
   "In this visualization, changes in volume through a piston, heating and cooling, contact with a heat bath, and changes in external pressure move the system between equilibrium states that can be followed on thermodynamic-function surfaces. In the quasistatic limit, the process itself may be regarded as a continuous trajectory through equilibrium states. For an irreversible process driven by a finite temperature or pressure difference, however, intermediate states need not be equilibrium states, so points on the surface primarily represent the equilibrium states before and after the operation. The ledger displays the heat \\(Q\\) absorbed by the system and the work \\(W\\) done by the system on its surroundings, allowing the first law to be checked using the sign convention \\(\\Delta U=Q-W\\)."],
  ["水を模したモデルでは、液相・気相の共存と相転移も観察できます。二相共存領域では、液体と気体が同じ温度と圧力で平衡し、全体のモル体積やモルエントロピー、モル内部エネルギーは相分率に応じて変化します。第一種相転移では熱力学ポテンシャル自体は連続でも、その一次微分が不連続になることがあります。たとえば Gibbs 自由エネルギーでは \\(s=-(\\partial g/\\partial T)_P\\)、\\(v=(\\partial g/\\partial P)_T\\) なので、相境界を横切るとエントロピーや体積の不連続として現れます。このように、相転移は熱力学関数の曲面やその傾きに幾何学的な特徴として現れます。体積・エントロピー・エネルギーなどの示量量は、\\(v\\)、\\(s\\)、\\(u\\)、\\(f\\)、\\(h\\)、\\(g\\) のような 1 mol あたりのモル量として表示します。",
   "The water-like model also allows liquid-vapor coexistence and phase transitions to be explored. In the two-phase region, liquid and vapor coexist in equilibrium at the same temperature and pressure, while the overall molar volume, molar entropy, and molar internal energy vary with the phase fraction. At a first-order phase transition, a thermodynamic potential can remain continuous while its first derivatives become discontinuous. For the Gibbs free energy, for example, \\(s=-(\\partial g/\\partial T)_P\\) and \\(v=(\\partial g/\\partial P)_T\\), so crossing a phase boundary can appear as discontinuities in entropy or volume. Phase transitions therefore leave geometric signatures in thermodynamic surfaces and their slopes. Extensive quantities such as volume, entropy, and energy are displayed as molar quantities per mole, denoted by \\(v\\), \\(s\\), \\(u\\), \\(f\\), \\(h\\), and \\(g\\)."],
  ...
  ["状態欄は境界条件によらず \\(T,P,v,s,u,f,h,g\\) を表示します。表示単位は示量変数について 1 mol あたりです。この試作は単一成分の純物質なので、化学ポテンシャルは Gibbs 自由エネルギーに等しく、\\(\\mu=g\\) です。相共存中も平衡にある各相の \\(\\mu\\) は同じ値です。", "The state panel always shows \\(T,P,v,s,u,f,h,g\\). Extensive quantities are reported per mole. For this pure one-component model, chemical potential equals molar Gibbs energy, \\(\\mu=g\\), and every coexisting equilibrium phase has the same \\(\\mu\\)."],
  ...
  ["共存点ではレバー則で相の割合を求めます。\\(g(T,P)\\) の折れ目では各相側の極限平面を示し、混合状態の平面は「支持平面」として表示します。三重点では Helmholtz 関数やエンタルピーにも非一意な傾きが現れる場合があり、同様に極限の平面を示します。三重点を温度・圧力だけで直接指定した場合は、数値的に選ばれた一相を表示します。", "At coexistence, phase fractions follow the lever rule. At folds of \\(g(T,P)\\), limiting planes from each phase are shown, while the mixture plane is a supporting plane. Helmholtz energy and enthalpy can likewise have nonunique slopes at the triple point. Selecting the triple point using only temperature and pressure displays the phase chosen by the numerical tie-break."],
]);
