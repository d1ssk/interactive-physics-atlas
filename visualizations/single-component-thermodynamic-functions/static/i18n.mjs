...
const STATIC_EN = new Map([
  ...
  ["状況に応じて、自然変数の異なる熱力学ポテンシャルを使うと便利です。モル Helmholtz 自由エネルギー \\(f=u-Ts\\)、モルエンタルピー \\(h=u+Pv\\)、モル Gibbs 自由エネルギー \\(g=u-Ts+Pv\\) は、\\(u(s,v)\\) の変数をルジャンドル変換によって交換したものです。それぞれ \\(df=-s\\,dT-P\\,dv\\)、\\(dh=T\\,ds+v\\,dP\\)、\\(dg=-s\\,dT+v\\,dP\\) を満たし、\\(f(T,v)\\)、\\(h(s,P)\\)、\\(g(T,P)\\) の曲面の傾きから、エントロピー・圧力・温度・体積といった共役な熱力学量を読み取れます。どのポテンシャルが使いやすいかは、温度、圧力、体積、エントロピーのうち何を外部から制御するかによって変わります。",
