...
const STRINGS = {
  en: {
    ...
    conditionAdiabatic: "Insulated walls. Piston motion is isentropic, and the wall is fixed afterward. Volume is fixed during heating or cooling. Ambient temperature and external pressure are not controls.",
    ...
    ledgerControl: "Local heating / cooling", ledgerBath: "Exchange with heat bath", ledgerNet: "Net heat Q", ledgerWork: "work W", ledgerDeltaU: "Change in internal energy ΔU", ledgerScope: "Ledger for the full n = 1.000 mol system",
    ...
    followed: "The local surface followed the current point. Its position is outlined in purple on the global view.",
  ...
  ja: {
    ...
    conditionAdiabatic: "断熱壁。ピストン操作は等エントロピーで、操作後は壁を固定します。加熱・冷却中は体積を固定します。外気温と外圧は操作しません。",
    ...
    ledgerControl: "局所加熱・冷却", ledgerBath: "熱浴との交換", ledgerNet: "正味の熱 Q", ledgerWork: "仕事 W", ledgerDeltaU: "内部エネルギーの変化 ΔU", ledgerScope: "n = 1.000 mol の系全体の収支",
    ...
    followed: "局所曲面の表示範囲が現在点を追従しました。全体図の紫の線で位置を確認できます。", changed: "準静的な操作で平衡状態を変更しました。収支は直前の操作分です。",
...
const STATIC_EN = new Map([
  ...
  ["ピストンを動かす。熱を加える。その操作を、熱力学関数の上の一歩として見る。", "Move the piston. Add heat. See each operation as a step across a thermodynamic-function surface."],
  ["示量変数は 1 mol あたりで表示し、操作は準静的な平衡状態の変化として扱います。", "Extensive quantities are shown per mole, and operations are treated as quasistatic changes between equilibrium states."],
  ...
  ["直前の操作の収支", "Latest-operation ledger"], ["操作すると熱と仕事を表示します。", "Operate on the state to display heat and work."], ["\\(\\Delta U=Q-W\\) · 系へ入る熱と、系が外へする仕事を正", "\\(\\Delta U=Q-W\\) · Heat entering the system and work done by it are positive"],
  ["エンタルピー", "Enthalpy"], ["現在点に表示範囲を合わせる", "Fit range to current point"], ["GLOBAL / 全体", "GLOBAL"], ["紫の線", "Purple outline"], ["= メイン表示の範囲", "= main display range"], ["現在の平衡状態", "Current equilibrium state"],
