...
const MESSAGES = {
  en: {
    ...
    timeHelp: "One atomic time unit is 24.19 as. The lowest selected energy \\(E_{\\rm ref}\\) is held fixed, and each component evolves as \\(\\exp[-i(E_n-E_{\\rm ref})t/\\hbar]\\).",
    ...
    phaseLabel: "Relative phase \\(\\arg\\widetilde\\psi\\)",
    norm: "Norm",
    ...
    detailPhase: "Hue represents complex phase relative to the lowest selected energy; point placement and brightness represent probability density. The unobservable common dynamical phase is removed, so an energy eigenstate is fully stationary.",
  ...
  ja: {
    ...
    resetTime: "時刻を戻す",
    playbackSpeed: "再生速度",
    timeHelp: "1原子時間は24.19 asです。選択中の最低エネルギー \\(E_{\\rm ref}\\) を固定し、各成分を \\(\\exp[-i(E_n-E_{\\rm ref})t/\\hbar]\\) に従って発展させます。",
    displayHeading: "表示",
    pointCount: "点の数",
    ...
    spinOff: "自動回転：オフ",
    spinOn: "自動回転：オン",
    phaseLabel: "相対位相 \\(\\arg\\widetilde\\psi\\)",
    norm: "規格化",
    ...
    detailPhase: "色相は選択中の最低エネルギーを基準にした複素位相、点の配置と明るさは確率密度を表します。観測できない共通の動的位相は除いているため、エネルギー固有状態は色相も含めて静止します。",
